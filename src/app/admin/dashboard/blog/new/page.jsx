...
export default function NewBlogPost() {
  ...
  const [loading, setLoading] = useState({ auth: true, post: false });
  ...
  const [uploadStates, setUploadStates] = useState({ image: 0, thumb: 0 });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await updateDoc(doc(firestore, "posts", `${Date.now()}`), {
        ...formData,
        categories: formData.categories.map(c => c.value),
        tags: formData.tags.map(t => t.value),
        meta: { title: formData.metaTitle, description: formData.metaDesc, keywords: formData.keywords },
        featuredImage: { url: formData.featuredImage?.url || '', alt: formData.featuredImage?.alt || `${formData.title} thumbnail` },
      ...
      await Swal.fire("Success", "Post created!", "success");
      router.push("/admin/dashboard/blog");
    } catch (error) {
      Swal.fire("Error", error.message, "error");
    }
  };
  if (loading.auth) return <LoadingSpinner />;
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8">
          ...
          <div className="space-y-6">
            <PublishControl status={formData.status} setStatus={(status) => setFormData({ ...formData, status })} onSubmit={handleSubmit} />
            <CategoryTagSelect categories={options.categories} tags={options.tags} formData={formData} setFormData={setFormData} />
            <FeaturedImageUpload formData={formData} handleUpload={(file) => handleUpload(file, 'blog-thumbnails')} uploadProgress={uploadStates["blog-thumbnails"]} setFormData={setFormData} />
